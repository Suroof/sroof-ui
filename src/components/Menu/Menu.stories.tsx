...
const meta: Meta<typeof Menu> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '一个功能丰富的菜单组件，支持水平、垂直和内联模式，以及多种主题样式。'
      }
    }
  },
  argTypes: {
  ...
  render: (args) => (
    <Menu {...args}>
      ...
      <SubMenu key="sub1" itemKey="sub1" title="设置" icon={<SettingsIcon />}>
        <MenuItem key="3" itemKey="3">基本设置</MenuItem>
        <MenuItem key="4" itemKey="4">高级设置</MenuItem>
        <MenuItem key="5" itemKey="5">安全设置</MenuItem>
      </SubMenu>
  ...
  render: (args) => (
    <div style={{ height: '500px', display: 'flex' }}>
      <Menu {...args}>
        ...
        <SubMenu key="sub1" title="用户管理" icon={<UserIcon />}>
          <MenuItem key="2">用户列表</MenuItem>
          <MenuItem key="3">角色管理</MenuItem>
          <MenuItem key="4">权限设置</MenuItem>
        </SubMenu>
        <SubMenu key="sub2" title="系统设置" icon={<SettingsIcon />}>
          <MenuItem key="5">基本配置</MenuItem>
          <MenuItem key="6">安全设置</MenuItem>
          <MenuItem key="7">日志管理</MenuItem>
        </SubMenu>
        <MenuItem key="8" icon={<DocumentIcon />}>
          文档中心
        </MenuItem>
        <MenuItem key="9" icon={<HelpIcon />}>
          帮助支持
        </MenuItem>
        <MenuItem key="10" danger>
          系统重置
        </MenuItem>
      </Menu>
    </div>
