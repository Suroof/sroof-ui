...
function AlertWithIconExample() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      <Alert type="info" showIcon message="这是一条带图标的信息提示" />
      <Alert type="success" showIcon message="这是一条带图标的成功提示" />
      <Alert type="warning" showIcon message="这是一条带图标的警告提示" />
      <Alert type="error" showIcon message="这是一条带图标的错误提示" />
    </div>
  );
}
```
